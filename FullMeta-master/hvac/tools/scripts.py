...
class PklRecorder:
    ...
    def collect(self, state, action, reward, done, next_state):
        self.traj["observations"].append(state)
        self.traj["actions"].append(action)
        self.traj["rewards"].append(reward)
        self.traj["terminals"].append(done)
        self.traj["next_observations"].append(next_state)

    def data_transfer_to_array(self):
        data_array = {
            "actions": np.array(self.traj["actions"], dtype=np.float32),
            "observations": np.array(self.traj["observations"], dtype=np.float32),
            "next_observations": np.array(self.traj["next_observations"], dtype=np.float32),
            "rewards": np.array(self.traj["rewards"], dtype=np.float32),
            "terminals": np.array(self.traj["terminals"])
        }
        return data_array
...
class DataRecorder:
    ...
    def collect(self, info, reward,home):
        if self._on_recording:
            ...
            for id in map(int, id_list):
                ...
                if unrealHome[id].people.is_in_room(hour):
