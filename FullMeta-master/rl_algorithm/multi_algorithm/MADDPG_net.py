...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        max_mem = min(self.capacity, self.current_memo_size)
        if max_mem < self.batch_size:
            idxes = range(0, self.current_memo_size)
        else:
            idxes = np.random.choice(max_mem, self.batch_size)
        # Single + batch
        obses = torch.tensor(self.obs_cap[idxes], dtype=torch.float).to(device)
        next_obses = torch.tensor(self.next_obs_cap[idxes], dtype=torch.float).to(device)
        states= torch.tensor(self.state_cap[idxes], dtype=torch.float).to(device)
        next_states = torch.tensor(self.next_state_cap[idxes], dtype=torch.float).to(device)
        actions = torch.tensor(self.action_cap[idxes], dtype=torch.float).to(device)
        rewards = torch.tensor(self.reward_cap[idxes], dtype=torch.float).to(device)
        dones = torch.tensor(self.done_cap[idxes], dtype=torch.float).to(device)
