...
class Net(nn.Module):
    def __init__(self, name, chkpt_dir="../scripts/data/tmp/2024073120008",
                 best_chkpt_dir="../scripts/data/tmp/best"):
        super().__init__()
        ...
        self.best_checkpoint_file = os.path.join(self.best_checkpoint_dir, name + '_td3')
    ...
    def load_best_checkpoint(self):
        self.load_state_dict(T.load(self.best_checkpoint_file))
...
class RDQN(object):
    def __init__(self, chkpt_dir="", best_chkpt_dir=""):
        self.network, self.target_network = Net(name="network", chkpt_dir=chkpt_dir, best_chkpt_dir=best_chkpt_dir).to(
            device), Net(name="target_network", chkpt_dir=chkpt_dir, best_chkpt_dir=best_chkpt_dir).to(device)
    ...
    def save_best_models(self):
        self.network.save_best_checkpoint()
        self.target_network.save_best_checkpoint()
    ...
    def load_best_models(self):
        self.network.load_best_checkpoint()
        self.target_network.load_best_checkpoint()
