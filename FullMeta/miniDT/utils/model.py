...
from miniDT.utils.config import MAX_TIMESTEP
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.attention(x)  # residual 残差连接
        x = self.ln1(x)
...
class DecisionTransformer(nn.Module):
    def __init__(self, state_dim, act_dim, n_blocks, h_dim, context_len,
                 n_heads, drop_p, max_timestep=MAX_TIMESTEP):
        super().__init__()
    ...
    def forward(self, timesteps, states, actions, returns_to_go):
        B, T, _ = states.shape  #B:64 T:60 _:23
